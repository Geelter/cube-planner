...
create table cube_cards (
    ...
    -- >= 0 (not >= 1): CHECKs are per-statement and never deferrable, so the
    -- RemoveCubeCardQuantity → DeleteDepletedCubeCards two-statement pattern
    -- needs a transient 0 to be legal (execution fix, Task 2).
